...
- [Issue #5](https://github.com/pettarin/volumito/issues/5): async REST API client
  ...
  to use it, install the `volumito[async]` extra
- Install extra `volumito[all]`, providing both the `async` and the `scp` extras
...
- `queue list` shows the title of the local files, which the Volumio host reports
  under `name`
...
- Reviewed and updated other documentation
...
- Command `queue status`, printing the current track with the position, the length, and
  the neighbor flags of the queue, honoring the usual fields and format options
- Read-only property `queue_status` on the REST API client

### Changed

- The dotted fields selected with `-L`/`--fields` print inside their rebuilt nesting
  (e.g., `data: {value: ...}`), instead of as flat dotted keys
...
- The kept-download and file-already-exists messages double-quote their paths
...
- The identifiers that can contain spaces (names, paths, URIs) are double-quoted in the messages
...
- `scp put` copies only when `-y`/`--yes` is given
...
- Subcommand `system execute`, running a command on the Volumio host over SSH, guarded
  by `-y`/`--yes` and exiting with the code the command returned

### Changed

- The SSH failures raise `VolumioSSHError`, of which `VolumioSCPError` is now a subclass
...
- Command group `notifications`, with the `list`, `listen`, `register`, and `unregister`
  ...
  `register` and `unregister` compose the URL of the local listener with `-A`, and
