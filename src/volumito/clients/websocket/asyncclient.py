...
class VolumioAsyncWebSocketClient(VolumioWebSocketCommon):
    ...
    async def add_and_play(self, uri: str) -> None:
        ...
        await self._emit(EVENT_ADD_PLAY, self._queue_uri_item(uri))
    ...
    async def add_radio_favourite(
        ...
        await self._request(EVENT_ADD_TO_FAVOURITES, EVENT_URI_FAVOURITES, payload)
    ...
    async def add_to_queue(self, uri: str) -> None:
        ...
        await self._emit(EVENT_ADD_TO_QUEUE, self._queue_uri_item(uri))
    ...
    async def check_update_cache(self) -> UpdateCheck:
        ...
        Raises:
            ...
            VolumioAPIError: If the answer is not an object
        """
        return UpdateCheck.from_raw(await self._read_object(EVENT_UPDATE_CHECK_CACHE))
    ...
    async def get_power_modes(self) -> PowerModes:
        ...
        A host that reports no standby mode ignores :meth:`standby`.
    ...
    async def get_usb_drives(self) -> UsbDrives:
        ...
        Raises:
            ...
            VolumioAPIError: If the answer is not an object
        """
        results = await self.browse(USB_BROWSE_URI)
        return UsbDrives.from_raw({"drives": [item.raw for item in results.items]})
    ...
    async def remove_radio_favourite(self, uri: str) -> None:
        ...
        await self._request(EVENT_REMOVE_FROM_FAVOURITES, EVENT_PUSH_BROWSE_LIBRARY, payload)
    ...
    async def safe_remove_drive(self, name: str) -> None:
        ...
        Raises:
            VolumioConnectionError: If not connected, or if the event cannot be sent
        """
        await self._emit(EVENT_SAFE_REMOVE_DRIVE, name)
    ...
    async def set_background(self, name: str) -> None:
        ...
        Args:
            name: The name of the image, from :meth:`get_backgrounds`, or a colour
        ...
        """
        if name.startswith("#"):
            await self._emit(EVENT_SET_BACKGROUNDS, self._background_color_payload(name))
            return
    ...
    async def standby(self) -> None:
        ...
        A host whose :meth:`get_power_modes` report no standby mode ignores the request,
